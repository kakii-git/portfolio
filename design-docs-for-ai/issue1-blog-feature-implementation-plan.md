...
| 実装ブランチ（予定） | `feat/issue-1-blog`（`main` から切る） |
| 状態 | Reviewed（自己レビュー3回 + 独立レビュアーによる外部レビュー、§10 参照）。初回記事は AI 生成の検証用仮記事で確認後に削除する方針で確定（§3.11）。計画書自体は未コミットのため、着手前に Phase 0 の手順0で `main` へ入れる |
...
8. AI 生成の検証用記事で受け入れ条件を確認し削除、README を整えて PR（`Closes #1`）
...
0. **計画書を `main` に入れる**。現時点で `design-docs-for-ai/` は未追跡（`git status` が `??` を返す）。`plan/issue-1-blog-feature` 上で `git add design-docs-for-ai/` → `docs: Issue #1 ブログ機能の実装計画を追加` でコミット → push → PR → `main` へマージする。置き場所は `design-docs-for-ai/` のまま（AI 向け設計文書の置き場として新設。人間向けの `docs/adr/` とは分ける）
...
PR は `feat/issue-1-blog` → `main`。本文に §6.3 の diff 結果と §6.4 の表をチェックリストとして貼る。`Closes #1`。

本計画書は `plan/issue-1-blog-feature` ブランチの作業ツリーにあり、**まだコミットされていない**。Phase 0 の手順0でこのブランチにコミットし、PR で `main` へマージしてから実装ブランチを切る（実装中に計画書と実装が同じブランチで混ざらないようにするため）。実装中に計画から逸脱した点は、実装ブランチ側でこの文書の §10 に「実装時の逸脱」として追記する。
